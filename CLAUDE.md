...
### Backup Process (scripts/backups.sh)

**Key Functions:**
- `backup_db()`: Main backup loop, handles both full database dumps and per-table dumps
- `dump_tables()`: Extracts individual table schemas/data when `DB_TABLES=TRUE`
- `remove_files()`: Cleanup old backups respecting `REMOVE_BEFORE` and `MIN_SAVED_FILE`
- `clean_s3bucket()`: S3-specific cleanup using `s3cmd`

**Backup Flow:**
1. Creates year/month directory structure: `/${BUCKET}/${YEAR}/${MONTH}/`
...
**Monitoring:**
- `HEALTHCHECKS_URL`: Optional URL to call when backup completes successfully (e.g., https://hc-ping.com/your-uuid)
...
- S3-specific: `BUCKET`, `ACCESS_KEY_ID`, `SECRET_ACCESS_KEY`, `HOST_BASE`, `HOST_BUCKET`, `SSL_SECURE`, `DEFAULT_REGION`
...
- The backup scripts use ANSI color codes (`\e[1;31m`, `\033[0m`) for console output
- Cleanup logic (scripts/backups.sh:147-169) sorts files by modification time to preserve newest files when enforcing `MIN_SAVED_FILE`
- S3 cleanup (scripts/backups.sh:26-46) iterates through bucket contents and compares timestamps
...
- Table dumps query `information_schema.tables` excluding system schemas (pg_catalog, information_schema, topology)
